...
func loadCodewalk(fs vfs.Opener, abspath, filename string) (*Codewalk, error) {
	codewalkXML := filepath.Join(abspath, filename)
	f, err := fs.Open(codewalkXML)
	if err != nil {
		return nil, err
	}
	defer f.Close()
	...
	err = d.Decode(cw)
	if err != nil {
		return nil, &os.PathError{Op: "parsing", Path: codewalkXML, Err: err}
	}
	...
	for _, st := range cw.Step {
		...
		filename := filepath.Join(abspath, st.Src[0:i])
